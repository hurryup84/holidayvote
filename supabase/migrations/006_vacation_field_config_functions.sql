-- HolidayVote V6 - Vacation field configuration (fixed)
-- Functions to manage per-vacation field configuration

-- Drop existing functions first (in case they exist from previous attempts)
DROP FUNCTION IF EXISTS public.upsert_vacation_field_config(UUID, TEXT, BOOLEAN, INTEGER);
DROP FUNCTION IF EXISTS public.get_vacation_field_config(UUID);
DROP FUNCTION IF EXISTS public.init_vacation_field_config(UUID);

-- Drop existing policies first
DROP POLICY IF EXISTS "Owners can manage field config" ON public.vacation_field_config;
DROP POLICY IF EXISTS "Participants can view field config" ON public.vacation_field_config;
...
-- Policy: owners can manage field configs
...
-- Policy: participants can view field configs
...
-- Function to upsert field config for a vacation
...
AS $$
BEGIN
  -- Verify user is owner of the vacation
  IF NOT EXISTS (
